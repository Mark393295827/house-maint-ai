    ...
    try {
        ...
        try {
            ...
            for (const report of allReports) {
                ...
                } catch (e) { console.error('Error inserting test review:', e); }
            }
            ...
            if (isSQLite) {
                ...
                } catch (e) { console.error('Error altering patterns table (performance_score):', e); }
                try {
                    await pool.query(`ALTER TABLE patterns ADD COLUMN generation_version INTEGER DEFAULT 1`);
                } catch (e) { console.error('Error altering patterns table (generation_version):', e); }
            } else {
                await pool.query(`
            ...
            if (isSQLite) {
                ...
                } catch (e) { console.error('Error altering reports table (pattern_id):', e); }
            } else {
                await pool.query(`
                    ALTER TABLE reports 
            ...
            for (const col of newColumns) {
                if (isSQLite) {
                    ...
                    } catch (e) { console.error(`Error adding column ${col.name} to reports table:`, e); }
                } else {
                    await pool.query(`
                        ALTER TABLE reports 
            ...
            for (const col of patternColumns) {
                if (isSQLite) {
                    ...
                    } catch (e) { console.error(`Error adding column ${col.name} to patterns table:`, e); }
                } else {
                    await pool.query(`
                        ALTER TABLE patterns 
            ...
            for (const col of workerColumns) {
                if (isSQLite) {
                    ...
                    } catch (e) { console.error(`Error adding column ${col.name} to workers table:`, e); }
                } else {
                    await pool.query(`
                        ALTER TABLE workers 
                        ADD COLUMN IF NOT EXISTS ${col.name} ${col.type}
