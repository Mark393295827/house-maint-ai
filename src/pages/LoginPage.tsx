...
type SocialProvider = 'wechat' | 'alipay' | 'google' | 'apple';
...
const LoginPage = () => {
    ...
    const handleSocialLogin = (provider: SocialProvider) => {
        showToast(
            isZh ? `${provider === 'wechat' ? '微信' : provider === 'alipay' ? '支付宝' : provider} 登录即将上线` : `${provider.charAt(0).toUpperCase() + provider.slice(1)} login coming soon`,
            'info'
        );
    };
                ...
                <div className="w-full max-w-sm">
                    ...
                    <div className="flex flex-col items-center gap-6 stagger-item">
                        ...
                        <div className="w-full grid grid-cols-2 gap-3">
                             {isZh ? (
                                <>
                                    <button type="button" onClick={() => handleSocialLogin('wechat')} className="h-12 bg-white border border-black/5 rounded-xl flex items-center justify-center gap-3 hover:shadow-md transition-all press-scale relative">
                                ...
                                <>
                                    ...
                                    <button type="button" onClick={() => handleSocialLogin('apple')} className="h-12 bg-black rounded-xl flex items-center justify-center gap-3 hover:shadow-xl transition-all press-scale relative">
                                         ...
                                         <span className="absolute top-1 right-1 text-[8px] font-bold text-white/60 bg-white/10 px-1 rounded">Soon</span>
                                    </button>
                                </>
                             )}
                        </div>
