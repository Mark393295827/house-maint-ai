...
const urgencyBadge: Record<number, { key: string; color: string; border: string }> = {
    ...
    1: { key: 'mid', color: 'text-[#007aff]', border: 'border-[#007aff]/10' },
...
const getJobStatusTone = (status: string) => {
    if (status === 'in_progress') return 'bg-[#007aff]/10 text-[#007aff] border border-[#007aff]/20';
    if (status === 'completed') return 'bg-[#28cd41]/10 text-[#28cd41] border border-[#28cd41]/20';
...
const WorkerDashboardPage: React.FC = () => {
    ...
    const statsData = [
        { icon: 'payments', label: t('workerPortal.stats.earnings'), value: `¥${stats.earnings.toFixed(0)}`, accent: '#28cd41' },
        { icon: 'task_alt', label: t('workerPortal.stats.completed'), value: stats.jobsCompleted, accent: '#007aff' },
                ...
                <div className="flex items-center justify-between gap-3 max-w-lg mx-auto">
                    ...
                    <div className="flex shrink-0 items-center gap-2">
                        ...
                        <button
                            ...
                            className="h-10 w-10 rounded-full bg-white border border-black/10 text-[#1d1d1f] shadow-sm flex items-center justify-center transition-all hover:bg-black/5 disabled:opacity-40"
                        >
                        ...
                        <button
                            ...
                            className={`group relative h-10 px-3 sm:px-5 rounded-full font-black text-[11px] tracking-widest uppercase transition-all flex items-center gap-2 whitespace-nowrap ${available ? 'bg-[#1d1d1f] text-white shadow-xl shadow-black/10' : 'bg-white border border-black/10 text-[#1d1d1f]'}`}
                        >
            ...
            <div className="px-6 py-4 max-w-lg mx-auto stagger-item">
                <div className="p-1.5 apple-glass bg-white/40 ring-1 ring-black/5 rounded-2xl flex gap-1">
                    <button
                        ...
                        className={`flex-1 min-w-0 h-11 rounded-xl text-[12px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${tab === 'available' ? 'bg-[#1d1d1f] text-white shadow-lg' : 'text-[#86868b] hover:bg-black/5'}`}
                    >
                    ...
                    <button
                        ...
                        className={`flex-1 min-w-0 h-11 rounded-xl text-[12px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all ${tab === 'myJobs' ? 'bg-[#1d1d1f] text-white shadow-lg' : 'text-[#86868b] hover:bg-black/5'}`}
                    >
            ...
            <main className="px-6 max-w-lg mx-auto">
                {notification && (
                    <div className="mb-6 p-4 apple-glass border border-black/5 rounded-2xl flex items-center gap-3 animate-in fade-in slide-in-from-top-4 duration-500 shadow-xl shadow-black/5" role="status">
                        <div className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center ${notification.type === 'success' ? 'bg-[#28cd41]/10 text-[#28cd41]' : 'bg-[#ff3b30]/10 text-[#ff3b30]'}`}>
                            <span className="material-symbols-outlined text-lg">{notification.type === 'success' ? 'check_circle' : 'error'}</span>
                        </div>
                        <span className="text-[14px] font-bold text-[#1d1d1f]">{t(notification.key, notification.params)}</span>
                    </div>
                )}
